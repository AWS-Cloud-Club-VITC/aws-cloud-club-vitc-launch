  ...
  try {
    ...
    if (!name || !regno || !phoneNumber || !vitEmail) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: name, regno, phoneNumber, or vitEmail",
        },
        { status: 400 },
      );
    }
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Preference 1 and Preference 2 cannot have the same department",
        },
        { status: 400 },
      );
    }
    ...
    if (!personalQuestions?.q1 || !personalQuestions?.q2) {
      return NextResponse.json(
    ...
    const coll = db.collection("awsrecruitment26");
    ...
    const recruitmentData: RecruitmentFormData = {
      ...
      preference1: {
        ...
        ...(deptConfig[preference1.dept]?.project
          ? { projects: preference1.projects?.trim() }
          : {}),
        ...(deptConfig[preference1.dept]?.projectLink
          ? { projectLink: preference1.projectLink?.trim() }
          : {}),
        ...(deptConfig[preference1.dept]?.github
