...
export default function RecruitmentApplyPage() {
  ...
  const validateForm = (): string | null => {
    ...
    if (!validateEmail(formData.vitEmail))
    ...
    const validatePreferenceHelper = (
      ...
      for (let i = 0; i < config.questions.length; i++) {
        ...
        if (!answer || !answer.trim()) {
          ...
          const qShort =
            config.questions[i].length > 30
              ? config.questions[i].substring(0, 30) + "..."
              : config.questions[i];
    ...
    const p1Error = validatePreferenceHelper(
      formData.preference1,
      "Preference 1",
    );
    if (p1Error) return p1Error;

    const p2Error = validatePreferenceHelper(
    ...
    if (
      formData.preference1.dept &&
      formData.preference2.dept &&
      formData.preference1.dept === formData.preference2.dept
    ) {
      return "Preference 1 and Preference 2 cannot have the same department";
    }

    // Personal questions validation
    if (!formData.personalQuestions.q1.trim())
      return "All personal questions are required";
    if (!formData.personalQuestions.q2.trim())
      return "All personal questions are required";
        ...
        <div className="space-y-2">
          ...
          >
            ...
            {technicalDepts.map((dept) => {
              const isDisabled =
                pref === "preference2" && formData.preference1.dept === dept;
              return (
                <option
                  key={dept}
                  value={dept}
          ...
          {pref === "preference2" && formData.preference1.dept && (
            <p className="text-xs text-yellow-500 mt-1">
              Note: {formData.preference1.dept} is already selected in
              Preference 1
            </p>
          )}
        </div>
          ...
          return (
            <>
              {config.project && (
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      updatePreference(pref, "projects", e.target.value)
                    }
              ...
              {config.projectLink && (
                <div className="space-y-2">
                  <Label htmlFor={`${pref}-projectLink`} className="text-white">
                    {config.projectLink.label}{" "}
                    {config.projectLink.required && (
                      <span className="text-yellow-500">*</span>
                    )}
                  </Label>
                  <Textarea
                    ...
                    onChange={(e) =>
                      updatePreference(pref, "projectLink", e.target.value)
                    }
              ...
              {config.github && (
                <div className="space-y-2">
                  <Label
                    htmlFor={`${pref}-githubProfile`}
                    className="text-white"
                  >
                    GitHub Profile{" "}
                    {config.github.required && (
                      <span className="text-yellow-500">*</span>
                    )}
                  </Label>
                  <Input
                    ...
                    onChange={(e) =>
                      updatePreference(pref, "githubProfile", e.target.value)
                    }
      ...
      <div className="container mx-auto max-w-7xl">
        ...
        >
          ...
          <h1 className="text-4xl md:text-5xl font-bold font-heading text-gradient mb-4">
            Join AWS Student Builder Group VIT-C
          </h1>
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 mb-6">
            <p className="text-yellow-400 text-sm font-semibold">
              ⚠️ Important: If you submit multiple times, only the most recent
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, vitEmail: e.target.value })
                  }
            ...
            <CardContent className="space-y-8 md:space-y-6">
              {personalQuestions.map((question, idx) => (
                <div key={idx} className="space-y-2">
                  ...
                  <Textarea
                    value={
                      formData.personalQuestions[`q${idx + 1}` as "q1" | "q2"]
                    }
                    onChange={(e) =>
